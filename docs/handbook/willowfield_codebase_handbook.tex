...
\begin{document}
% Digital handbook: do not insert blank pages to force right-hand openings.
...
\begin{titlepage}
	...
	\begin{tcolorbox}[width=0.94\textwidth,colback=white,colframe=CodeBorder,boxrule=0.7pt,arc=2mm,left=4mm,right=4mm,top=3mm,bottom=3mm]
		...
		\href{https://github.com/Sivothajan/csc3081_farm_project/tree/695efa55384de87452d60e0aaabfff137de02fd4}{\texttt{Sivothajan/csc3081\_farm\_project}}\par\vspace{0.15cm}
		\noindent\makebox[0.36\linewidth][l]{\bfseries\color{Ink}Project author:}%
		Sivothayan .S -- S/21/513\par\vspace{0.15cm}
		\noindent\makebox[0.36\linewidth][l]{\bfseries\color{Ink}Code snapshot:}%
		\code{main @ 695efa5} -- 8 September 2026\par\vspace{0.15cm}
		\noindent\makebox[0.36\linewidth][l]{\bfseries\color{Ink}Full source commit:}%
		{\scriptsize\ttfamily 695efa55384de87452d60e0aaabfff137de02fd4}\par\vspace{0.15cm}
		\noindent\makebox[0.36\linewidth][l]{\bfseries\color{Ink}Release:}%
		\href{https://github.com/Sivothajan/csc3081_farm_project/releases/tag/v0.2.0}{Mid Review -- v0.2.0}\par
	\end{tcolorbox}

	\vfill
	{\footnotesize\color{Muted} This handbook documents the pushed code snapshot above. The release also includes the subsequently updated handbook source and PDF.}
\end{titlepage}
...
\begin{itemize}
	\item Every C++ subsystem under \file{src/}: animals, characters, core, environment, objects, structures, systems, utilities, and vegetation.
	...
	\item Wind deformation, grass cutting, crops, trees, cow animation, herd state transitions, barn doors, farmer walking and night patrols.
...
Willowfield is an interactive 3D farm built using C++20, classic fixed-function OpenGL, GLU and FreeGLUT. The farm contains terrain, dirt and grass areas, paths, fences, a barn, windmill, hay bales, rocks, crops, trees, animated grass, cows, an articulated farmer, a day/night transition, lighting, textures, signs, a custom handwritten font and a HUD.
...
\begin{center}
	\begin{tikzpicture}[
		...
		\node[sys,below=of scene] (systems) {Systems\\wind/herd/farmer/cutting};
...
\begin{longtable}{@{}p{0.31\textwidth}p{0.63\textwidth}@{}}
	...
	\pathrow{src/characters/}{Farmer geometry, distance-driven walking, day/night routes and lantern.}
...
The scene owns a camera plus terrain, path, fence, windmill, barn, rocks, hay, lighting, wind, grass, crops, trees, herd, farmer, animation state, cutting system, HUD, textures and sky. It also stores a short-lived text-reload notice. The shared text catalog itself is global through the \code{Text} namespace. This means \code{Scene} remains the natural place to add a new top-level farm component.
...
\end{lstlisting}
\code{Text::initialize()} first resolves the editable \file{assets/text.txt}; grass and crops then generate deterministic procedural layouts. Shared textures, the farmer head bitmap and the font atlas locate runtime assets relative to the executable, with a fallback to a local \file{assets} directory.
...
\begin{center}
	\begin{tikzpicture}[node distance=6mm, every node/.style={font=\small}, arr/.style={-{Latex},thick,Muted}]
		...
		\node[draw,rounded corners,below=of herd] (farmer) {Farmer pose + follow camera};
		\node[draw,rounded corners,below=of farmer] (sky) {Sky transition};
		\node[draw,rounded corners,below=of sky] (cut) {Grass cutting};
		\draw[arr] (anim)--(wind); \draw[arr] (wind)--(mill); \draw[arr] (mill)--(herd); \draw[arr] (herd)--(farmer); \draw[arr] (farmer)--(sky); \draw[arr] (sky)--(cut);
	\end{tikzpicture}
\end{center}
Before animated systems advance, \code{textNoticeTime} is reduced so the F9 success/failure message disappears after five seconds. \code{AnimationSystem::update()} returns zero while paused, so downstream animated systems stop naturally. The farmer receives \code{farmer.update(step, sky.night)}. If follow mode is active, \code{Scene} refreshes the camera from the new farmer position and forward vector before updating the sky.
...
\begin{longtable}{@{}lll@{}}
	...
	\keycap{J}      & Follow/release farmer camera   & Camera/Farmer      \\
	\keycap{U}      & Stop/resume farmer            & Farmer             \\
	...
	\keycap{N}      & Toggle requested night        & Sky/Herd/Farmer    \\
...
The render function applies global, barn and farmer-lantern lighting, selects wireframe/fill mode, then draws sky, terrain, path, fence, windmill, barn, rocks, hay, vegetation, herd, farmer, cutting indicator and finally HUD.
...
\section{Following the farmer}
\keycap{J} toggles a separate follow mode, leaving the eight numbered presets unchanged. The camera's \code{followFarmer()} method places it 4.2 units ahead, 2.2 units to the side and 2.65 units above the farmer's ground position. It aims 1.4 units above that position, showing the face and clothing while the farmer turns and walks.

\code{Scene::update()} refreshes this view after the farmer pose changes. Pressing \keycap{J} again, moving/looking manually or choosing another view releases it. The label is \code{view.farmer} in \file{assets/text.txt}.
...
\section{Farmer lantern}
\code{Farmer::light()} uses \code{GL\_LIGHT2} for a warm point light at the character's lantern. The scene applies it before drawing world geometry. Its intensity follows \code{nightAmount}, and the global lighting toggle \keycap{L} disables it along with the other lights. The farmer carries the glowing lantern while night is requested, including the approach to the barn. Requesting morning removes the lantern and disables its light.
...
These four shared materials are separate from the farmer's head texture and the handwriting atlas. \code{Farmer} owns its head texture ID directly, so adding the farmer did not add another \code{Surface} enum value.

\section{Farmer bitmap and attribution}
The farmer's generic face uses \textbf{TheNess}'s \href{https://opengameart.org/content/human-male-body-and-head-texture-painted-with-adobe-animate}{Human male body and head texture (painted with Adobe Animate)}, published on OpenGameArt under \href{https://creativecommons.org/publicdomain/zero/1.0/}{CC0 1.0 Universal}. The original \href{https://opengameart.org/sites/default/files/head_texture.png}{head\_texture.png} is converted to an uncompressed 24-bit BMP, retaining the complete 150 by 150 image.

\begin{itemize}
	\item Runtime bitmap: \file{assets/textures/characters/farmer_head.bmp}.
	\item Source, original-file checksum and conversion notes: \file{assets/textures/characters/SOURCE.txt}.
	\item Full license: \file{assets/textures/characters/CC0-1.0.txt}.
	\item Reproducible import: \file{scripts/import-farmer-texture.ps1}.
\end{itemize}

\code{Farmer::head()} maps the painted face region through UV coordinates onto a curved mesh. Hair, ears, hat, clothing, limbs and animation are procedural project code. No personal photograph or external human model is used. With textures disabled or the bitmap unavailable, modeled facial features provide a fallback. The asset is bundled; normal builds and runtime do not fetch it from the internet. See also \file{docs/THIRD_PARTY.md} for the complete resource inventory.
...
\file{scripts/generate-textures.py} procedurally creates the four 64x64 24-bit BMP material textures with fixed random seeds 3081--3084. That generator needs no third-party images or Python packages and does not modify the farmer bitmap.
...
User-facing wording is no longer scattered across \file{Hud.cpp}, \file{Camera.cpp}, \file{Path.cpp}, \file{Barn.cpp}, \file{Herd.cpp}, \file{Farmer.cpp} and \file{Cow.cpp}. The repository now has one plain-text catalog at \file{assets/text.txt}. A beginner can rename the farm, signs, stalls, views and status messages without recompiling C++.
...
\begin{longtable}{@{}p{0.27\textwidth}p{0.63\textwidth}@{}}
	...
	\code{farmer.*}                          & Walking, crop inspection, stopped, transfer and guarding messages. \\
...
\code{HudState} contains cutting status, pause status, whether the structural study is shown, cut-blade count, camera height, herd-status text, farmer-status text, sleeping-cow count, day/night, camera-view name and an optional text-reload notice.
...
\begin{itemize}
	\item top-left: farm title, author line, localized view/day/asleep status, herd or pause status, and farmer activity;
...
Nearly all visible wording comes through \code{Text::get()} or \code{Text::format()}. This includes title, author, view status, herd status, farmer status, wind labels, help, cutting state and study labels. Edit \file{assets/text.txt} rather than hardcoding new strings in \file{Hud.cpp}.
...
\begin{longtable}{@{}p{0.32\textwidth}p{0.58\textwidth}@{}}
	...
	Top-left panel size      & \code{leftWidth} and the \code{150} panel height.                                            \\
...
\section{Layout}
\file{Crop.h} declares a shared \code{rowCenters} array with 11 X coordinates:
\begin{lstlisting}[language=C++]
static constexpr std::array<float, 11> rowCenters{
    -7.3f, -6.15f, -5.0f, -3.85f, -2.7f, 3.05f,
    4.2f, 5.35f, 6.5f, 7.65f, 8.8f};
\end{lstlisting}
\code{Crop::initialize()} iterates this array. The central gap keeps the main path clear. Z positions run from about -20.2 to -4.4 with 0.42 spacing. \code{Farmer::makeRoute()} uses adjacent-row midpoints for four walking aisles; this keeps the path and stalk placement tied to the same coordinates.
...
\begin{editbox}
	To increase crop density, edit \code{Crop::rowCenters} and/or reduce the Z step .42. Keep the array sorted, review the farmer aisle indices and clearance, and update the separately generated terrain furrows so the plants still sit on their rows.
\end{editbox}
...
\part{Characters, Animals and Simulation Systems}
...
\chapter[Farmer: Walking and Night Patrol]{Farmer: Walking, Crop Inspection and Night Patrol}
\label{ch:farmer}
\section{Files and responsibility}
\file{src/characters/Farmer.h} declares the public update, render, lighting, position, forward-vector, pause and status APIs. \file{Farmer.cpp} builds the routes, evaluates the pose and draws the character. \code{Scene} owns one \code{Farmer} and starts it automatically.

\begin{beginnerbox}
	The same character has two jobs: tour the farm during the day and patrol the cow barn at night. The requested time of day is changed with \keycap{N}; there is no automatic day/night clock. Walking, stops and travel between the two routes happen automatically.
\end{beginnerbox}

\section{Model and outfit}
The farmer is assembled from fitted meshes and drawing helpers: a straw hat, checked shirt, denim overalls with straps and buttons, a pocket, hands with fingers and work boots. \code{tailored()} connects elliptical rings instead of using a stack of boxes. Alternating quad colors form the checked shirt pattern. The head includes modeled hair, ears and a curved surface for the generic painted face.

The online BMP and its CC0 source are documented in the texture chapter. The body, outfit, lantern and animation are implemented locally; no personal photograph or downloaded human model is used.

\section{State and route samples}
\begin{lstlisting}[language=C++]
struct Sample {
    Vec3 point;
...
The constructor builds \code{dayRoute} and \code{nightRoute}, then starts on the day circuit. \code{route} stores the active loop or temporary connecting path. Each sample records a world point, arrival time and accumulated path distance. A repeated point with a later time represents a stop. The double-precision \code{clock} follows the current route; \code{completedDistance} preserves gait continuity when a circuit wraps.
...
\begin{longtable}{@{}p{0.23\textwidth}p{0.67\textwidth}@{}}
	...
	\midrule
	\endhead
	\code{Day} & Repeat the farm tour and inspect wheat. \\
	\code{ToBarn} & Retrace the current circuit to its entrance and follow the connecting path to the barn. \\
	\code{Night} & Walk around all four exterior sides of the barn, with two lookout stops. \\
	\code{ToFields} & Leave the patrol and walk back to the daytime circuit. \\
	\bottomrule
\end{longtable}

\section{Daytime destinations}
\code{makeRoute(false)} defines a closed sequence of corner points starting at the farm center. The route visits the central and windmill paths, both wheat plots, the barn yard, meadow and pasture. Four crop aisles are computed from selected adjacent entries in \code{Crop::rowCenters}. The pasture route crosses the actual fence opening near X = 3, Z = 15.

Most corners use a quadratic curve:
\[
P(u)=(1-u)^2E+2u(1-u)C+u^2X, \qquad 0\leq u\leq1.
\]
Here \(E\) is the corner entry, \(C\) the original corner, and \(X\) the exit. Sixteen samples approximate each curve. The radius is limited to 0.65 units and a quarter of each neighboring segment, keeping turns inside narrow aisles. Two wheat inspection points add 4.5-second stops with 0.55-second speed ramps before/after each stop.

\section{Night patrol and continuous transfers}
\code{makeRoute(true)} places a loop outside the barn, spanning X = -23 to -11.8 and Z = -14 to -1.2. The farmer carries a lantern and pauses at two points to look around. This guarding behavior is a patrol animation; the scene has no predator or combat simulation.

\code{transfer()} starts from the exact current position, retraces the shorter part of the active circuit to its entrance, then appends the connecting path between the farm center and barn. This preserves the crop aisles and pasture gate instead of moving straight across obstacles. \code{activate()} starts the destination loop. Heading blends over 0.8 seconds at a route change.

If \keycap{N} changes again while the farmer is transferring, he finishes that passage first and then honors the latest request. Returning to morning restores the complete daytime tour. The update loop can consume a large time step across a transfer boundary, which keeps deterministic captures consistent with ordinary playback.

\begin{warningbox}
	These are predefined routes, not general pathfinding or collision detection. Moving a barn wall, gate, crop row or path requires reviewing the corresponding corner lists and connecting paths. The farmer avoids stalk roots; wind-blown leaves can still brush the character.
\end{warningbox}

\section{Distance-driven walk cycle}
\begin{lstlisting}[language=C++]
constexpr float speed = 1.12f, stride = 1.32f;
gait = float(std::fmod(travelled / stride, 1.0)) * 2 * Constants::PI;
\end{lstlisting}

Speed is measured in world units per second; stride is the distance for one complete gait cycle. \code{pose()} finds the surrounding route samples using \code{upper_bound}, evaluates acceleration/deceleration, interpolates position and looks slightly ahead to determine heading. Local +Z is the farmer's forward direction, so heading uses \code{atan2(direction.x, direction.z)}.

In the renderer, feet alternate between ground contact and a lifted swing. \code{kneeBetween()} solves a two-segment leg from the hip and ankle positions, using upper/lower lengths 0.59 and 0.56. The body bobs and arms swing with the gait. Inspection pauses fade in head movement and a reaching gesture. At night the right hand carries the lantern while the left arm remains animated.

\section{Controls, camera and HUD}
\begin{itemize}
	\item \keycap{U} toggles \code{walkingEnabled}, stopping only the farmer.
	\item \keycap{P} pauses shared animation, including farmer, cows and doors.
	\item \keycap{J} follows the farmer; manual camera input or another view releases it.
	\item \keycap{N} requests the other day/night routine for farmer and herd.
	\item \keycap{T} switches between the textured and modeled face; \keycap{L} controls scene lighting, including the lantern.
\end{itemize}

\code{statusKey()} returns a \code{farmer.*} key for walking, wheat rows, inspecting, stopped, heading to the barn, returning to the fields or guarding. \code{Scene::render()} resolves that key and passes it as \code{HudState::farmerStatus}. Change wording in \file{assets/text.txt} and reload with \keycap{F9}.

\section{Checking the character}
The behavior suite checks closed circuits, crop clearance, inspection stops, timing, all four night-patrol sides, transfer continuity, pause/resume and follow-camera controls. Render cases cover day, night, morning, wheat rows, meadow, pasture, small windows, wireframe and textures off. Close face captures check that the bitmap maps correctly onto the head.

\begin{figure}[p]
	\centering
	\includegraphics[width=0.96\textwidth,height=0.34\textheight,keepaspectratio]{screenshots/farmer-day.png}
	\par\vspace{8mm}
	\includegraphics[width=0.96\textwidth,height=0.34\textheight,keepaspectratio]{screenshots/farmer-night.png}
	\caption{Farmer follow views: daytime outfit and walking pose (top), and a lantern near the cow barn during the night routine (bottom).}
\end{figure}
...
\begin{longtable}{@{}lp{0.55\textwidth}@{}}
	...
	\code{--view NAME}          & Select a farm preset, \code{farmer} follow view or \code{farmer-face} close-up. \\
	...
	\code{--morning}            & Advance 60 seconds of night, request morning, then apply the selected time advance.                           \\
...
The \code{farmer} view frames the character after the requested simulation advance. Choose \code{farmer-face} for head texture inspection. \code{--morning} changes both herd and farmer targets; it does not assume that the farmer has already completed his return path.
...
\begin{itemize}
	...
	\item Farmer loops and inspection stops are deterministic; routes clear crop stalks and use the pasture gate.
	\item Night patrol covers all four barn sides; day/night transfers remain continuous, including repeated requests.
	\item Farmer motion is consistent across update rates, respects both pause controls and supports follow-camera release.
\end{itemize}
...
Runs an extensive Release/x64 verification suite. It can skip rebuilding and can refresh the README screenshot. It captures many cases: overview, still/windy meadow, cutting, crops, night, barn entry, sleeping, morning, empty barn, side barn views, small window, no-HUD, no-study, front/back/left/right, cows, wireframe and no-textures. Farmer captures add close face, crop inspection, meadow, pasture, night patrol, morning return, small-window, wireframe and untextured views. It also benchmarks 180 frames, tests native window shutdown, tests startup with no assets, and checks the editable text-file behavior.

For the farmer update, Windows Release/x64 verification passed 83 behavior checks, 36 render cases, five native window-close runs, missing-assets startup and the 180-frame benchmark. Successful rendering checks OpenGL errors and process status; appearance still requires visual inspection.
...
\section{Farmer texture import}
\file{scripts/import-farmer-texture.ps1} downloads the pinned source PNG, verifies its SHA-256 and uses Windows System.Drawing to write a 24-bit BMP with the same 150 by 150 pixels. Explicit source/destination pixel rectangles avoid DPI-based scaling. Source and CC0 notices remain beside the bitmap. The importer is optional because the runtime asset is already included.

\section{Rebuilding this handbook}
Run \code{pdflatex} from the repository root so the farmer screenshot paths resolve. Use \file{build/handbook/} for intermediate files, compile three passes to settle contents and cross-references, then copy the PDF to \file{docs/handbook/}. \file{docs/SCRIPTS.md} contains the exact PowerShell commands. Review the final log and rendered pages; the repository formatter does not process LaTeX.
...
\begin{longtable}{@{}p{0.24\textwidth}p{0.23\textwidth}p{0.14\textwidth}p{0.31\textwidth}@{}}
	...
	User-facing labels         & \file{assets/text.txt}                  & key/value catalog & Farm title, author, views, herd, farmer, help, signs, stalls and status wording; reload with F9. \\
	...
	Farmer walk speed          & \file{Farmer.cpp}: \code{speed}         & 1.12 units/s      & Travel speed along sampled routes. \\
	Farmer gait distance       & \file{Farmer.cpp}: \code{stride}        & 1.32 units        & Distance per full walk cycle. \\
	Farmer stop duration       & \code{makeRoute()}                      & 4.5 s             & Two stops in each loop. \\
	Farmer speed ramps         & \code{makeRoute()}                      & 0.55 s            & Ease into and out of stops. \\
	Crop X coordinates         & \file{Crop.h}: \code{rowCenters}        & 11 rows           & Shared with farmer aisle calculation. \\
...
\begin{longtable}{@{}p{0.25\textwidth}p{0.34\textwidth}p{0.33\textwidth}@{}}
	...
	Farmer shirt    & \rgb{.64}{.23}{.14}                 & \file{characters/Farmer.cpp}   \\
	Farmer overalls & \rgb{.12}{.26}{.37}                 & \file{characters/Farmer.cpp}   \\
...
\begin{tabularx}{\textwidth}{@{}lXX@{}}
	...
	Medium & Windmill, trough, crop field extents  & May overlap farmer paths; inspect route clearance.                       \\
	High   & Barn, pasture gate, central paths     & Herd/farmer routes, cameras, fences and verification depend on them. \\
	\bottomrule
\end{tabularx}
...
\section{Farmer}
Change \code{speed} for travel pace and \code{stride} for distance per gait cycle in \file{Farmer.cpp}. Route corners and stop durations are in \code{makeRoute()}; day/night connecting points are in \code{transfer()}. Review crop-aisle indices and gate crossings if destinations move. Chapter~\ref{ch:farmer} explains the pose and route states.

\section{Day/night}
The \keycap{N} key changes the target; time of day does not cycle automatically. Sky visual fade and herd/farmer behavior triggers are conceptually separate. \code{sky.night} immediately changes the requested behavioral state, while \code{nightAmount} visually eases over time.
...
The crop generator reads the 11 X values in \code{Crop::rowCenters}. The farmer derives four aisle centers from selected adjacent values. Terrain furrows still use a separate approximately 1.15-spaced loop. Update these together, preserve the central path and run the farmer clearance checks.

\chapter{Changing the Barn and Cow Routine}
\section{Move the barn or pasture gate}
Update barn geometry and its coordinate helpers, herd lanes, camera presets, fences and the farmer's night corners/transfer points together. The farmer's day route also uses the pasture gate. A moved wall or closed opening can invalidate a previously clear predefined route.
...
Edit \code{farm.author} and \code{farm.title} in \file{assets/text.txt}.

\section{Camera, herd and farmer wording}
Edit \code{view.*}, \code{herd.*} and \code{farmer.*}. These labels are resolved through the text catalog, so wording changes do not touch movement/behavior code. \code{view.farmer} names the follow camera, while \code{help.views} includes the \keycap{J/U} controls.
...
The safest method is to preserve the same file name, location and 24-bit uncompressed BMP format. For tiled materials, keep a modest size such as 64x64 or 128x128. The farmer head is 150x150; preserve its layout or update the facial UV mapping as well.

\section{Add a fifth shared surface type}
The farmer head is loaded separately from this shared surface enum. To extend the enum, you must:
\begin{enumerate}
...
\section{Farmer stops or has not reached the barn}
Read the farmer activity row. \keycap{U} stops him, \keycap{P} pauses everyone, and scheduled stops last 4.5 seconds. A day/night transfer retraces the current loop before changing routes; repeated requests finish that transfer first.
...
Create \file{FarmLayout.h} for barn center, windmill center, pasture bounds, gate position and crop field bounds. Herd, farmer, camera and render modules could then share these coordinates. \code{Crop::rowCenters} already shares crop X positions with farmer aisle generation, but terrain furrows and other layout values remain separate.
...
Where practical, derive cow and stall counts from their containers or configuration instead of hardcoding \code{3}. The previous hardcoded \code{2300} HUD total has already been removed, and user-facing wording has already moved to \file{assets/text.txt}.
...
\begin{longtable}{@{}p{0.36\textwidth}p{0.56\textwidth}@{}}
	...
	\pathrow{src/characters/Farmer.h}{Farmer route samples, routine state, pause, pose and rendering/light APIs.}
	\pathrow{src/characters/Farmer.cpp}{Day tour, night patrol, continuous transfers, articulated geometry, face texture and lantern.}
	\pathrow{src/core/Camera.h}{Camera state and view enum.}
	\pathrow{src/core/Camera.cpp}{Movement, look, view transform, presets and farmer follow/release mode.}
	...
	\pathrow{src/vegetation/Crop.h}{Crop stalk data, shared rowCenters coordinates and interface.}
...
\begin{longtable}{@{}p{0.42\textwidth}p{0.50\textwidth}@{}}
	...
	\pathrow{scripts/import-farmer-texture.ps1}{Checks the online head PNG and converts it to a 24-bit BMP.}
	...
	\pathrow{docs/THIRD\_PARTY.md}{FreeGLUT, Patrick Hand and TheNess head-texture credits, licenses and references.}
	\pathrow{docs/handbook/willowfield_codebase_handbook.tex}{LaTeX source for this handbook.}
	\pathrow{docs/handbook/willowfield_codebase_handbook.pdf}{Compiled handbook.}
	\pathrow{assets/textures/characters/farmer_head.bmp}{Generic painted head texture, converted to BMP.}
	\pathrow{assets/textures/characters/SOURCE.txt}{Head texture author, source, checksum and conversion notes.}
	\pathrow{assets/textures/characters/CC0-1.0.txt}{Full CC0 1.0 license for the head texture.}
	\pathrow{screenshots/farmer-day.png}{Daytime farmer follow-view reference.}
	\pathrow{screenshots/farmer-night.png}{Farmer with lantern near the barn at night.}
...
\begin{longtable}{@{}ll@{}}
	...
	V                  & Overview                                \\
	H                  & Barn interior                           \\
	J                  & Follow/release farmer camera             \\
...
# Farmer captures (day / night / morning)
...
\begin{center}
	...
	\href{https://github.com/Sivothajan/csc3081_farm_project/tree/695efa55384de87452d60e0aaabfff137de02fd4}{\texttt{Sivothajan/csc3081\_farm\_project @ 695efa5}}\\
	\code{main} branch code snapshot -- 8 September 2026.\\
	\href{https://github.com/Sivothajan/csc3081_farm_project/releases/tag/v0.2.0}{Mid Review release -- v0.2.0}.
\end{center}
